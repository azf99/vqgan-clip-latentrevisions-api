...
class StyleCLIP(object):
    def __init__(self, prompt, uid, output_path = './styleclip_out', img_path = None):
      ...
      self.latents_init = torch.zeros(self.latent_shape).squeeze(-1).to(self.device)
    ...
    def truncation(self, x, threshold=0.7, max_layer=8):
        avg_latent = torch.zeros(self.batch_size, x.size(1), 512).to(self.device)
    ...
    def run(self):
        counter = 0
        path_to_res = ""
        while counter <= self.img_save_freq:
